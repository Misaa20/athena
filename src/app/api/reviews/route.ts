...
export async function POST(req: Request) {
  ...
  const { rating, body, spoiler, bookId, book } = parsed.data;
  ...
  const review = await db.review.upsert({
    ...
    update: { rating, body, spoiler: spoiler ?? false },
    create: { userId: user.id, bookId: resolvedBookId, rating, body, spoiler: spoiler ?? false },
  });
